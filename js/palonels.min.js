 ...
 * 
 * @namespace PANOLENS
 * @customer web-scavenger
 */

// ============================================================================================================================
//  this is custom variant of Panolens.js Library
// it was made just for this tusk. Now we can Made 360 3D mode with 2 blocks (2 viewwers)
// add some parametres for enableEffect() second true/false to remove/add screen point and third(optional) true if you want just hide point but use it 
// many fixes you can search by Ctrl+f and write 'was'. It`s something like crutches))
// added 3 param for PANOLENS.ImagePanorama if you want add callback()
...
THREE.DeviceOrientationControls = function (camera, domElement) {
	...
	this.update = function (ignoreUpdate) {
		...
		if (scope.enabled === false) return;
		// console.log('DEVICEORIENT') // try to use this one
...
GSVPANO.PanoLoader = function (parameters) {
	...
	PANOLENS.DataImage = {
		...
		ReticleIdle: '../img/reticle-idle.png', // was change for local image
	...
	PANOLENS.Utils.ImageLoader.load = function (url, onLoad, onProgress, onError) {
		...
		request.onloadend = function (event) {
			...
			if (panoramicTour.options.loadedCount == 1) {
				panoramicTour.options.leftImageLoadedCount++

				panoramicTour.options.progressLoad += 100 / (panoramicTour.options.leftEyeArray.length + 1);
				// console.log(panoramicTour.options.progressLoad) 
				// console.log(100 - (100 / (panoramicTour.options.leftImageLoadedCount - 1)))
				document.getElementById('js-percent_left').innerHTML = panoramicTour.options.progressLoad.toFixed(0) + '%';
				if (panoramicTour.options.leftImageLoadedCount - 1 == panoramicTour.options.leftEyeArray.length) {
					document.getElementById('js-percent_left').innerHTML = ''
					panoramicTour.initViewerBlock()
				}
			} else if (panoramicTour.options.loadedCount == 2) {
				panoramicTour.options.leftImageLoadedCount++;
				// console.log(panoramicTour.options.leftImageLoadedCount)
				if (panoramicTour.options.progressLoad < 99) {
					panoramicTour.options.progressLoad += 100 / (panoramicTour.options.leftEyeArray.length + panoramicTour.options.rightEyeArray.length + 1);
					document.getElementById('js-percent_left').innerHTML = panoramicTour.options.progressLoad.toFixed(0) + '%';
					document.getElementById('js-percent_right').innerHTML = panoramicTour.options.progressLoad.toFixed(0) + '%';
				} else {
					document.getElementById('js-percent_left').innerHTML = ''
					document.getElementById('js-percent_right').innerHTML = ''
				}
				// console.log(panoramicTour.options.progressLoad) 

				if (panoramicTour.options.leftImageLoadedCount - 1 == (panoramicTour.options.leftEyeArray.length + panoramicTour.options.rightEyeArray.length)) {
					document.getElementById('js-percent_left').innerHTML = ''
					document.getElementById('js-percent_right').innerHTML = ''
					panoramicTour.initViewerBlock()
				}
	 ...
	 */
	PANOLENS.Utils.TextureLoader.load = function (url, onLoad, onProgress, onError, preloadCount) {
		...
		if (preloadCount == 'left') {

		} else if (preloadCount == 'right') {
			panoramicTour.options.rightImageLoadedCount++
		}
		return texture;
...
PANOLENS.StereographicShader = {
	...
	PANOLENS.Panorama.prototype.onLoad = function () {
		...
		console.log('Main Room ready');
		// set timeout for delete image preloader
		panoramicTour.options.loadedPic++;
		// console.log(panoramicTour.options.imgData)


		console.log(panoramicTour.options.loadedPic)
		if (panoramicTour.options.loadedPic == panoramicTour.options.loadedCount) {
			setTimeout(function () {
				document.getElementById('js-preloader_right').style.display = 'none';

				document.getElementById('js-preloader_left').style.display = 'none';
		...
		// 	console.log('starts child room loading')
		// 	var childImg = self.children[i];	
		// 	PANOLENS.Utils.TextureLoader.load(childImg.toPanorama.src);
		// }
	 ...
	 */
	PANOLENS.ImagePanorama = function (image, radius, callback) {
	...
	PANOLENS.ImagePanorama.prototype.load = function (src) {
		src = src || this.src;
		// add preloader on screen
		console.log(panoramicTour.options.loadedPic)
		if (panoramicTour.options.loadedPic == panoramicTour.options.loadedCount) {
			panoramicTour.options.loadedPic = 0;
		}
		document.getElementById('js-preloader_left').style.display = 'flex';
		document.getElementById('js-preloader_right').style.display = 'flex';
		// console.log('loaded!!!')
	...
	*/
	 ...
	 * @param {number} 	[options.controlsMode]
	 ...
	 * 
	 */
	PANOLENS.Viewer.prototype.enableEffect = function (mode, controlsMode, hideCircle) {

		// controlsMode - param for control points at the screen: 
		// false if you want add control points
		// true if you want remove control points

		if (controlsMode) { this.disableReticleControl(); return; } //was add points 

		if (mode === PANOLENS.Modes.NORMAL) {
			if (hideCircle) {
				this.disableEffect(hideCircle);
			} else {
				this.disableEffect();
			}
			return;
	 ...
	 */
	PANOLENS.Viewer.prototype.disableEffect = function (hideCircle) {


		// if ( this.mode === PANOLENS.Modes.NORMAL ) { return; } was

		this.mode = PANOLENS.Modes.NORMAL;
		// this.disableReticleControl(); was
		this.enableReticleControl(hideCircle);
	 ...
	 */
	PANOLENS.Viewer.prototype.enableReticleControl = function (hideCircle) {
	...
	PANOLENS.Viewer.prototype.disableReticleControl = function () {
		...
		// this.tempEnableReticle = true;
		this.tempEnableReticle = false; //was

		// Register mouse event and unregister reticle event
		if (!this.options.enableReticle) {

			this.reticle.hide(); //was
			//this.reticle.show();
	 ...
	 */
	PANOLENS.Viewer.prototype.enableControl = function (index, firstCamera, secondCamera) {
		// console.log(this.options)
		// this.Viewer.controlsMode = index; // was. add for change mode
		// if (firstCamera && secondCamera) {

		// 	firstCamera.control.enabled = false;

		// 	firstCamera.control = firstCamera.controls[index];

		// 	firstCamera.control.enabled = true;

		// 	firstCamera.camera.position.copy(firstCamera.panorama.position);
		// 	firstCamera.camera.rotation.copy(firstCamera.panorama.rotation);
		// 	firstCamera.control.update();

		// 	secondCamera.control.enabled = false;

		// 	secondCamera.control = secondCamera.controls[index];

		// 	secondCamera.control.enabled = true;


		// 	secondCamera.camera.position.copy(firstCamera.panorama.position);

		// 	secondCamera.camera.quaternion.copy(firstCamera.camera.rotation)


		// 	// console.log(firstCamera.camera);
		// 	// console.log(secondCamera.camera);
		// 	secondCamera.control.update();
		// 	firstCamera.activateWidgetItem(index, undefined);
		// 	secondCamera.activateWidgetItem(index, undefined);
		...
		switch (index) {
			...
			case PANOLENS.Controls.ORBIT:
				console.log("ORBIT!!!!!!!!!!!!!!!!!!!!!!!!")
				//this.panorama.position  copy this
			...
			case PANOLENS.Controls.DEVICEORIENTATION:
				...
				// console.log("PANOLENS.Controls.DEVICEORIENTATION!!!!!!!!!!!!!!!!!!!")
				//this.panorama.position    copy this
				this.camera.position.copy(this.panorama.position);
	 ...
	 */
	PANOLENS.Viewer.prototype.onWindowResize = function (windowWidth, windowHeight, viewer) {
		...
		expand = self.container.classList.contains('panolens-container') || self.container.isFullscreen;

		if (windowWidth !== undefined && windowHeight !== undefined) {

			width = windowWidth;
			height = windowHeight;
			self.container._width = windowWidth;
			self.container._height = windowHeight;

		} else {

			width = expand ? Math.max(document.documentElement.clientWidth, window.innerWidth || 0) : self.container.clientWidth;
			height = expand ? Math.max(document.documentElement.clientHeight, window.innerHeight || 0) : self.container.clientHeight;
			self.container._width = width;
			self.container._height = height;
		...
		// Update reticle
		if (self.options.enableReticle || self.tempEnableReticle) {

			self.updateReticleEvent();
		 ...
		 */
		self.dispatchEvent({ type: 'window-resize', width: width, height: height });
	...
	PANOLENS.Viewer.prototype.onMouseUp = function (event) {
		...
		// was. change for off hide ifospors on screen click
		// if (type === 'click') {
